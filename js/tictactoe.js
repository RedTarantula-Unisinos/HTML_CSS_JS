...
{
    if(debugging) console.log("Clearing slots");
...
{
    if (turn == 0)
        document.getElementById('last_turn_display').innerHTML = '<b>Turn: X</b>'
    else
        document.getElementById('last_turn_display').innerHTML = '<b>Turn: O</b>';

}

function UpdateScores() // Draws the current score values
{
    if (debugging) console.log("Opdating scores");
...
{
    if (debugging) console.log("Checking availability");
...
{
    if (debugging) console.log("Switched turn");
    if (turn == 0) turn = 1;
    else turn = 0;
    UpdateTurnDisplay();
}
...
{
    if (debugging) console.log("Drawing slots values");
...
{
    if (debugging) console.log("Trying to take slot");
    if (!foundVictory && !foundTie) {
        var availability = CheckSlotAvailability(slot);
        if (debugging) console.log(availability);
...
{
    if (debugging) console.log("Checking for victory");
...
{
    if (debugging) console.log("Found victory");

    var lastgamestr = ' ';

    if (turn == 0) { scoreX++; turn = 0; lastgamestr = 'X XON!';}
    else if (turn == 1) { scoreO++; turn = 1; lastgamestr = 'O XON!';}
...
{
    if (debugging) console.log("Found tie");
    var lastgamestr = 'TIE!';
